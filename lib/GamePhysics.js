...
class PlayerState {
  ...
  snapshot() {
    return {
      ...
      fallen:   this.knockedUntilMs > Date.now(),
...
function updatePlayer(p, dt, trackLength, maxSpeed, nowMs = Date.now()) {
  if (p.finished) return;

  const step         = 1 / 60;
  const nitroActive  = (p.nitroUntilMs ?? 0) > nowMs;
  const fallen       = (p.knockedUntilMs ?? 0) > nowMs;
  ...
  if (!fallen) {
    ...
    // While downed, rapidly bleed speed and ignore controls.
    p.speed = accelerate(p.speed, -maxSpeed * 1.8, dt);
  }
